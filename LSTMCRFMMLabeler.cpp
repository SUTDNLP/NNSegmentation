...
void Labeler::readWordEmbeddings(const string& inFile, Alphabet& alphabet, NRMat<dtype>& wordEmb, bool needNormalized) {
  cerr<< "Reading embedding from file:" << inFile << endl; 
  static ifstream inf;
  if (inf.is_open()) {
    inf.close();
    inf.clear();
  ...
  while (1) {
    ...
    if (vecInfo.size() != wordDim + 1) {
      std::cout << "error embedding file"<<vecInfo.size() <<"\t"<<wordDim << std::endl;
    }
    if(needNormalized)
        curWord = normalize_to_lowerwithdigit(vecInfo[0]);
    else
...
int main(int argc, char* argv[]) {
	...
	ah.new_named_string("word", "wordEmbFile", "named_string",
			...
			wordEmbFile);
    ah.new_named_string("tag","tagEmbFile", "named_string", "pretrained [T**] embedding file list to train a model, using comma(,) as a delimiter, optional when training", tagEmbFileList); //[tzy]
