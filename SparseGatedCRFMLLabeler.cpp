...
#include "SparseGatedCRFMLLabeler.h"
...
void Labeler::train(const string& trainFile, const string& devFile, const string& testFile, 
  ...
  for (int idx = 0; idx < tagEmbs.size(); idx++) {
	...
	m_classifier.init(wordEmb, m_options.wordcontext, tagEmbs, m_labelAlphabet.size(), m_options.atomLayers, m_featAlphabet.size());
