...
#include "SparseLSTMCRFMMLabeler.h"
...
void Labeler::train(const string& trainFile, const string& devFile, const string& testFile, 
    const string& modelFile, const string& optionFile, const string& wordEmbFile, const string& tagEmbFileList) {
  if (optionFile != "")
  ...
  if (!m_options.wordEmbFineTune) {
    ...
    cout << "Remain words num in wordAlphabet: " << m_wordAlphabet.size() << endl;
  }
  if (!m_options.tagEmbFineTune) {
    addTestTagAlpha(devInsts);
    addTestTagAlpha(testInsts);
    for (int idx = 0; idx < otherInsts.size(); idx++) {
        addTestTagAlpha(otherInsts[idx]);
    }
    for( int idx = 0; idx < m_tagAlphabets.size(); idx++ )
        cout << "Remain words num in tagAlphabet"<< idx +1 << " : " << m_tagAlphabets[idx].size() << endl;
  }
  

  NRMat<dtype> wordEmb;
  if (wordEmbFile != "") {
    readWordEmbeddings(wordEmbFile, m_wordAlphabet, wordEmb);
  } else {
  ...
  if(tagEmbFileList != "")
  {
       vector<string> tagEmbFiles;     
       split_bychar(tagEmbFileList, tagEmbFiles, ',');
       for (int idx = 0; idx < tagEmbs.size(); idx++) {
            readWordEmbeddings(tagEmbFiles[idx], m_tagAlphabets[idx], tagEmbs[idx], false);
       }
  }else{
        for (int idx = 0; idx < tagEmbs.size(); idx++) {
  ...
  m_classifier.init(wordEmb, m_options.wordcontext, tagEmbs, m_labelAlphabet.size(), m_options.rnnHiddenSize, m_options.hiddenSize, m_featAlphabet.size());
...
void Labeler::readWordEmbeddings(const string& inFile, Alphabet& alphabet, NRMat<dtype>& wordEmb, bool needNormalized) {
  static ifstream inf;
  if (inf.is_open()) {
    inf.close();
    inf.clear();
  ...
  wordEmb.resize(alphabet.size(), wordDim);
  wordEmb = 0.0;
  if(needNormalized)
    curWord = normalize_to_lowerwithdigit(vecInfo[0]);
  else
    curWord = vecInfo[0];
  wordId = alphabet.from_string(curWord);
  ...
  std::cout << "OOV num is " << oovWords << ", total num is " << alphabet.size() << ", embedding oov ratio is " << oovWords * 1.0 / alphabet.size()
      << std::endl;
...
int main(int argc, char* argv[]) {
	...
	ah.new_named_string("word", "wordEmbFile", "named_string",
			...
			wordEmbFile);
    ah.new_named_string("tag","tagEmbFile", "named_string", "pretrained [T**] embedding file list to train a model, using comma(,) as a delimiter, optional when training", tagEmbFileList);
    ...
    std::cerr << tagEmbFileList << std::endl;

	Labeler tagger;
	if (bTrain) {
		tagger.train(trainFile, devFile, testFile, modelFile, optionFile, wordEmbFile, tagEmbFileList);
	} else {
