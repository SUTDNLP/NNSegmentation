/*
 * GRNNCRFMMClassifier.h
 *
 *  Created on: Mar 18, 2015
 *      Author: mszhang
 */

#ifndef SRC_GRNNCRFMMClassifier_H_
#define SRC_GRNNCRFMMClassifier_H_
...
public:
  ...
  GRNN<xpu> rnn_left_project;
  GRNN<xpu> rnn_right_project;
...
public:
  ...
  inline void init(const NRMat<dtype>& wordEmb, int wordcontext, const NRVec<NRMat<dtype> >& tagEmbs, int labelSize, int grnnhiddensize, int hiddensize) {
    ...
    _grnnhiddensize = grnnhiddensize;
    _token_representation_size = _wordDim + _tag_outputSize;
    _inputsize = _wordwindow * _token_representation_size;

    _words.initial(wordEmb);

    rnn_left_project.initial(_grnnhiddensize, _inputsize, true, 20);
    rnn_right_project.initial(_grnnhiddensize, _inputsize, false, 30);
    _tanh_project.initial(_hiddensize, 2 * _grnnhiddensize, true, 55, 0);
  ...
  inline dtype process(const vector<Example>& examples, int iter) {
    ...
    for (int count = 0; count < example_num; count++) {
      ...
      vector<Tensor<xpu, 2, dtype> > grnnoutput(seq_size), grnnoutputLoss(seq_size);
      vector<Tensor<xpu, 2, dtype> > mr_project_left(seq_size), r_project_left(seq_size), u_project_left(seq_size), c_project_left(seq_size);
      vector<Tensor<xpu, 2, dtype> > mr_project_right(seq_size), r_project_right(seq_size), u_project_right(seq_size), c_project_right(seq_size);
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        ...
        mr_project_left[idx] = NewTensor<xpu>(Shape2(1, _grnnhiddensize), d_zero);
        r_project_left[idx] = NewTensor<xpu>(Shape2(1, _grnnhiddensize), d_zero);
        u_project_left[idx] = NewTensor<xpu>(Shape2(1, _grnnhiddensize), d_zero);
        c_project_left[idx] = NewTensor<xpu>(Shape2(1, _grnnhiddensize), d_zero);
        project_left[idx] = NewTensor<xpu>(Shape2(1, _grnnhiddensize), d_zero);
        project_leftLoss[idx] = NewTensor<xpu>(Shape2(1, _grnnhiddensize), d_zero);

        mr_project_right[idx] = NewTensor<xpu>(Shape2(1, _grnnhiddensize), d_zero);
        r_project_right[idx] = NewTensor<xpu>(Shape2(1, _grnnhiddensize), d_zero);
        u_project_right[idx] = NewTensor<xpu>(Shape2(1, _grnnhiddensize), d_zero);
        c_project_right[idx] = NewTensor<xpu>(Shape2(1, _grnnhiddensize), d_zero);
        project_right[idx] = NewTensor<xpu>(Shape2(1, _grnnhiddensize), d_zero);
        project_rightLoss[idx] = NewTensor<xpu>(Shape2(1, _grnnhiddensize), d_zero);

        grnnoutput[idx] = NewTensor<xpu>(Shape2(1, 2 * _grnnhiddensize), d_zero);
        grnnoutputLoss[idx] = NewTensor<xpu>(Shape2(1, 2 * _grnnhiddensize), d_zero);        
      ...
      rnn_left_project.ComputeForwardScore(input, mr_project_left, r_project_left, u_project_left, c_project_left, project_left);
      rnn_right_project.ComputeForwardScore(input, mr_project_right, r_project_right, u_project_right, c_project_right, project_right);

      for (int idx = 0; idx < seq_size; idx++) {
        concat(project_left[idx], project_right[idx], grnnoutput[idx]);
      }
      _tanh_project.ComputeForwardScore(grnnoutput, project);
      ...
      _tanh_project.ComputeBackwardLoss(grnnoutput, project, projectLoss, grnnoutputLoss);
      for (int idx = 0; idx < seq_size; idx++) {
        unconcat(project_leftLoss[idx], project_rightLoss[idx], grnnoutputLoss[idx]);
      }


      // word combination
      rnn_left_project.ComputeBackwardLoss(input, mr_project_left, r_project_left, u_project_left, c_project_left, project_left, project_leftLoss, inputLoss);
      rnn_right_project.ComputeBackwardLoss(input, mr_project_right, r_project_right, u_project_right, c_project_right, project_right, project_rightLoss, inputLoss);
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        ...
        FreeSpace(&(mr_project_left[idx]));
        FreeSpace(&(r_project_left[idx]));
        FreeSpace(&(u_project_left[idx]));
        FreeSpace(&(c_project_left[idx]));
        FreeSpace(&(project_left[idx]));
        FreeSpace(&(project_leftLoss[idx]));

        FreeSpace(&(mr_project_right[idx]));
        FreeSpace(&(r_project_right[idx]));
        FreeSpace(&(u_project_right[idx]));
        FreeSpace(&(c_project_right[idx]));
        FreeSpace(&(project_right[idx]));
        FreeSpace(&(project_rightLoss[idx]));

        FreeSpace(&(grnnoutput[idx]));
      FreeSpace(&(project[idx]));
        FreeSpace(&(projectLoss[idx]));
        FreeSpace(&(grnnoutputLoss[idx]));
  ...
  void predict(const vector<Feature>& features, vector<int>& results) {
    ...
    vector<Tensor<xpu, 2, dtype> > grnnoutput(seq_size);
    vector<Tensor<xpu, 2, dtype> > mr_project_left(seq_size), r_project_left(seq_size), u_project_left(seq_size), c_project_left(seq_size);
    vector<Tensor<xpu, 2, dtype> > mr_project_right(seq_size), r_project_right(seq_size), u_project_right(seq_size), c_project_right(seq_size);
  ...
  void checkgrads(const vector<Example>& examples, int iter) {
    ...
    checkgrad(examples, rnn_left_project._rnn_update._WL, rnn_left_project._rnn_update._gradWL, "rnn_left_project._rnn_update._WL", iter);
    checkgrad(examples, rnn_left_project._rnn_update._WR, rnn_left_project._rnn_update._gradWR, "rnn_left_project._rnn_update._WR", iter);
    checkgrad(examples, rnn_left_project._rnn_update._b, rnn_left_project._rnn_update._gradb, "rnn_left_project._rnn_update._b", iter);
    checkgrad(examples, rnn_left_project._rnn_reset._WL, rnn_left_project._rnn_reset._gradWL, "rnn_left_project._rnn_reset._WL", iter);
    checkgrad(examples, rnn_left_project._rnn_reset._WR, rnn_left_project._rnn_reset._gradWR, "rnn_left_project._rnn_reset._WR", iter);
    checkgrad(examples, rnn_left_project._rnn_reset._b, rnn_left_project._rnn_reset._gradb, "rnn_left_project._rnn_reset._b", iter);
    ...
    checkgrad(examples, rnn_right_project._rnn_update._WL, rnn_right_project._rnn_update._gradWL, "rnn_right_project._rnn_update._WL", iter);
    checkgrad(examples, rnn_right_project._rnn_update._WR, rnn_right_project._rnn_update._gradWR, "rnn_right_project._rnn_update._WR", iter);
    checkgrad(examples, rnn_right_project._rnn_update._b, rnn_right_project._rnn_update._gradb, "rnn_right_project._rnn_update._b", iter);
    checkgrad(examples, rnn_right_project._rnn_reset._WL, rnn_right_project._rnn_reset._gradWL, "rnn_right_project._rnn_reset._WL", iter);
    checkgrad(examples, rnn_right_project._rnn_reset._WR, rnn_right_project._rnn_reset._gradWR, "rnn_right_project._rnn_reset._WR", iter);
    checkgrad(examples, rnn_right_project._rnn_reset._b, rnn_right_project._rnn_reset._gradb, "rnn_right_project._rnn_reset._b", iter);
...
#endif /* SRC_GRNNCRFMMClassifier_H_ */
