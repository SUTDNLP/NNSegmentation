/*
 * SparseGatedCRFMMClassifier.h
 *
 *  Created on: Mar 18, 2015
 *      Author: mszhang
 */

#ifndef SRC_SparseGatedCRFMMClassifier_H_
#define SRC_SparseGatedCRFMMClassifier_H_
...
class SparseGatedCRFMMClassifier {
public:
  SparseGatedCRFMMClassifier() {
    _dropOut = 0.5;
  }
  ~SparseGatedCRFMMClassifier() {
...
public:
  ...
  inline void init(const NRMat<dtype>& wordEmb, int wordcontext, const NRVec<NRMat<dtype> >& tagEmbs, int labelSize,
      int atom_composition_layer_num, int linearfeatSize) {
  ...
  inline dtype process(const vector<Example>& examples, int iter) {
    ...
    for (int count = 0; count < example_num; count++) {
      ...
      _olayer_linear.ComputeForwardScore(input, denseout);
// add sparse
      ...
      _olayer_linear.ComputeBackwardLoss(input, denseout, outputLoss, inputLoss);
...
#endif /* SRC_SparseGatedCRFMMClassifier_H_ */
